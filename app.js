...
function title(){return({home:'TM-Strojník',machines:'Moje stroje',machineForm:'Stroj',service:'Servisní kniha',inspections:'Prohlídky',inspectionForm:'Nová prohlídka',backup:'Záloha dat',logbook:'Provozní deník',logForm:'Nový záznam',logReport:'Výstup pro kontrolu',jobs:'Zakázky',jobForm:'Nová zakázka',fuel:'PHM a tankování',fuelForm:'Nové tankování'})[state.route]||'TM-Strojník'}
...
function fuel(){
 const liters=state.fuel.reduce((s,f)=>s+(Number(f.liters)||0),0);
 const cost=state.fuel.reduce((s,f)=>s+(Number(f.total)||0),0);
 return `<button class=primary data-go=fuelForm>＋ Přidat tankování</button>
...
function bind(){
 ...
 document.querySelectorAll('[data-delete]').forEach(x=>x.onclick=()=>{if(confirm('Odstranit stroj a jeho záznamy?')){const id=x.dataset.delete;state.machines=state.machines.filter(m=>m.id!==id);state.service=state.service.filter(s=>s.machineId!==id);state.inspections=state.inspections.filter(i=>i.machineId!==id);state.logbook=state.logbook.filter(l=>l.machineId!==id);save();render()}});
 ...
 document.querySelector('#fuelForm')?.addEventListener('submit',e=>{e.preventDefault();const f=new FormData(e.target);const liters=Number(f.get('liters'))||0;const pricePerLiter=Number(f.get('pricePerLiter'))||0;const hours=f.get('hours');state.fuel.unshift({id:crypto.randomUUID(),date:f.get('date'),machineId:f.get('machineId'),liters:String(liters),pricePerLiter:String(pricePerLiter),total:String(liters*pricePerLiter),hours,note:f.get('note').trim()});const m=state.machines.find(x=>x.id===f.get('machineId'));if(m&&hours)m.hours=hours;save();go('fuel')});
...
function render(){const view={home,machines,machineForm,service,inspections,inspectionForm,logbook,logForm,logReport,jobs,jobForm,fuel,fuelForm,backup}[state.route]||home;document.getElementById('app').innerHTML=shell(view());bind()}
render();