const KEYS={m:'tm3-machines',s:'tm3-service',i:'tm3-inspections',l:'tm3-logbook',j:'tm6-jobs',f:'tm6-fuel',q:'tm10-shifts',c:'tm11-consumables'};
const defaults=[
...
const state={route:'home',selected:'',machineSearch:'',machines:load(KEYS.m,defaults),service:load(KEYS.s,[]),inspections:load(KEYS.i,[]),logbook:load(KEYS.l,[]),jobs:load(KEYS.j,[]),fuel:load(KEYS.f,[]),shifts:load(KEYS.q,[]),consumables:load(KEYS.c,[])};
...
function title(){return({home:'TM-Strojník',machines:'Moje stroje',machineDetail:'Karta stroje',machineForm:'Stroj',service:'Servisní kniha',inspections:'Prohlídky',inspectionForm:'Nová prohlídka',backup:'Záloha dat',logbook:'Provozní deník',logForm:'Nový záznam',logReport:'Výstup pro kontrolu',jobs:'Zakázky',jobForm:'Nová zakázka',fuel:'PHM a tankování',fuelForm:'Nové tankování',shiftStart:'Začít směnu',shiftActive:'Probíhající směna',shiftEnd:'Ukončit směnu',shifts:'Směny',serviceSupplies:'Filtry a kapaliny',supplyForm:'Přidat položku',alerts:'Upozornění'})[state.route]||'TM-Strojník'}
...
function home(){
 ...
 const outOfService=state.machines.filter(m=>machineStatus(m).level==='bad').length;
 const warning=state.machines.filter(m=>machineStatus(m).level==='warn').length;
 const serviceDue=state.machines.filter(m=>{const r=machineStatus(m).serviceRemaining;return r!==null&&r<=50;}).length;
 const active=activeShift();
 const fuelThisMonth=state.fuel.filter(f=>String(f.date).slice(0,7)===new Date().toISOString().slice(0,7)).reduce((s,f)=>s+(Number(f.total)||0),0);
 return `${active?`<div class=active-shift-banner><div><small>Probíhající směna</small><b>${esc((state.machines.find(m=>m.id===active.machineId)?.brand||'')+' '+(state.machines.find(m=>m.id===active.machineId)?.model||''))}</b><span>${esc(active.operator)} · ${esc(active.job||'Bez zakázky')}</span></div><button class=shift-stop data-go=shiftEnd>Ukončit směnu</button></div>`:`<button class=shift-start data-go=shiftStart>▶ Začít směnu</button>`}<div class=stats><div class=stat><strong>${state.machines.length}</strong><span>Stroje</span></div><div class=stat><strong>${outOfService}</strong><span>Mimo provoz / po termínu</span></div><div class=stat><strong>${warning}</strong><span>Vyžadují pozornost</span></div></div><div class=summary-grid><div class=summary-box><small>Servisy do 50 MTH / po termínu</small><strong>${serviceDue}</strong></div><div class=summary-box><small>Prohlídky do 30 dnů</small><strong>${due}</strong></div><div class=summary-box><small>PHM tento měsíc</small><strong>${fuelThisMonth.toLocaleString('cs-CZ')} Kč</strong></div><div class=summary-box><small>Zakázky</small><strong>${state.jobs.length}</strong></div></div>
 <div class=tiles>
  ...
  ${tile('green','🕒','Směny','Historie práce obsluhy','shifts')}
  ${tile('red','⚠️','Upozornění','Servisy, revize a závady','alerts')}
  ${tile('gray','💾','Záloha dat','Export a obnovení','backup')}
  ${tile('gray','⚙️','Nastavení','Připravujeme','home')}
 </div><div class=footer>TM-Strojník v1.2</div>`;
}
...
function machineCard(m){
 ...
 <div class=actions><button class=dark data-machine-detail="${m.id}">Otevřít kartu</button><button class=fuel-button data-fuel-form="${m.id}">⛽ Tankování</button><button class=service-button data-service="${m.id}">🔧 Servis</button><button class=secondary data-edit="${m.id}">Upravit</button><button class=danger data-delete="${m.id}">Odstranit</button></div></article>`;
}
function machines(){
...
function shiftStart(){
 ...
 <div class=group><label>Stroj</label><select name=machineId required><option value="">Vyber stroj</option>${state.machines.map(m=>`<option value="${m.id}">${esc(m.brand)} ${esc(m.model)}</option>`).join('')}</select></div>
...
function bind(){
 ...
 document.querySelectorAll('[data-delete]').forEach(x=>x.onclick=()=>{if(confirm('Odstranit stroj a jeho záznamy?')){const id=x.dataset.delete;state.machines=state.machines.filter(m=>m.id!==id);state.service=state.service.filter(s=>s.machineId!==id);state.inspections=state.inspections.filter(i=>i.machineId!==id);state.logbook=state.logbook.filter(l=>l.machineId!==id);state.jobs=state.jobs.filter(j=>j.machineId!==id);state.fuel=state.fuel.filter(f=>f.machineId!==id);state.consumables=state.consumables.filter(c=>c.machineId!==id);save();render()}});
 ...
 save();go('shifts')});
 document.querySelector('#exportBtn')?.addEventListener('click',()=>{const blob=new Blob([JSON.stringify({version:'0.6',machines:state.machines,service:state.service,inspections:state.inspections,logbook:state.logbook,jobs:state.jobs,fuel:state.fuel,shifts:state.shifts,consumables:state.consumables},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='tm-strojnik-zaloha.json';a.click();URL.revokeObjectURL(a.href)});
 document.querySelector('#importFile')?.addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);state.machines=d.machines||[];state.service=d.service||[];state.inspections=d.inspections||[];state.logbook=d.logbook||[];state.jobs=d.jobs||[];state.fuel=d.fuel||[];state.shifts=d.shifts||[];state.consumables=d.consumables||[];save();alert('Záloha byla obnovena.');go('home')}catch{alert('Soubor není platný.')}};r.readAsText(file)});
 document.querySelector('#clearBtn')?.addEventListener('click',()=>{if(confirm('Opravdu vymazat všechna data?')){state.machines=[];state.service=[];state.inspections=[];state.logbook=[];state.jobs=[];state.fuel=[];state.shifts=[];state.consumables=[];save();go('home')}});
}
function render(){const view={home,machines,machineDetail,machineForm,service,inspections,inspectionForm,logbook,logForm,logReport,jobs,jobForm,fuel,fuelForm,shiftStart,shiftActive,shiftEnd,shifts,serviceSupplies,supplyForm,alerts,backup}[state.route]||home;document.getElementById('app').innerHTML=shell(view());bind()}
render();