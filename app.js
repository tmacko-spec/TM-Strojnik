...
function home(){
 ...
 <div class=footer>TM-Strojník v2.6.3</div>`;
}
...
function machineForm(){
 ...
 <div class=group>
  <label>Zařazení techniky *</label>
  <select name=category id=machineCategory required>
   <option value="">Vyber zařazení</option>
   <option value=machine ${category==='machine'?'selected':''}>🚜 Stavební stroj</option>
   <option value=vehicle ${category==='vehicle'?'selected':''}>🚐 Automobil</option>
  </select>
 </div>
...
function bind(){
 ...
 const machineCategory=document.querySelector('#machineCategory');
 if(machineCategory){
   machineCategory.onchange=()=>{
     const form=machineCategory.form;
     const vehicle=machineCategory.value==='vehicle';
     const setLabel=(name,text)=>{const el=form.elements[name];const label=el?.closest('.group')?.querySelector('label');if(label)label.textContent=text;};
     setLabel('type',vehicle?'Typ automobilu':'Typ stavebního stroje');
     setLabel('hours',vehicle?'Kilometry':'Motohodiny');
     setLabel('serial',vehicle?'VIN / SPZ':'Výrobní číslo / VIN');
     setLabel('serviceInterval',vehicle?'Servisní interval km':'Servisní interval MTH');
     setLabel('lastServiceHours',vehicle?'Poslední servis při km':'Poslední servis při MTH');
   };
 }
