const KEYS={m:'tm3-machines',s:'tm3-service',i:'tm3-inspections',l:'tm3-logbook',j:'tm6-jobs',f:'tm6-fuel',q:'tm10-shifts'};
const defaults=[
...
const state={route:'home',selected:'',machineSearch:'',machines:load(KEYS.m,defaults),service:load(KEYS.s,[]),inspections:load(KEYS.i,[]),logbook:load(KEYS.l,[]),jobs:load(KEYS.j,[]),fuel:load(KEYS.f,[]),shifts:load(KEYS.q,[])};
...
function title(){return({home:'TM-Strojník',machines:'Moje stroje',machineDetail:'Karta stroje',machineForm:'Stroj',service:'Servisní kniha',inspections:'Prohlídky',inspectionForm:'Nová prohlídka',backup:'Záloha dat',logbook:'Provozní deník',logForm:'Nový záznam',logReport:'Výstup pro kontrolu',jobs:'Zakázky',jobForm:'Nová zakázka',fuel:'PHM a tankování',fuelForm:'Nové tankování',shiftStart:'Začít směnu',shiftActive:'Probíhající směna',shiftEnd:'Ukončit směnu',shifts:'Směny'})[state.route]||'TM-Strojník'}
...
function home(){
 ...
 </div><div class=footer>TM-Strojník v1.0</div>`;
}
...
function bind(){
 ...
 save();go('shifts')});
 document.querySelector('#exportBtn')?.addEventListener('click',()=>{const blob=new Blob([JSON.stringify({version:'0.6',machines:state.machines,service:state.service,inspections:state.inspections,logbook:state.logbook,jobs:state.jobs,fuel:state.fuel,shifts:state.shifts},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='tm-strojnik-zaloha.json';a.click();URL.revokeObjectURL(a.href)});
 document.querySelector('#importFile')?.addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);state.machines=d.machines||[];state.service=d.service||[];state.inspections=d.inspections||[];state.logbook=d.logbook||[];state.jobs=d.jobs||[];state.fuel=d.fuel||[];state.shifts=d.shifts||[];save();alert('Záloha byla obnovena.');go('home')}catch{alert('Soubor není platný.')}};r.readAsText(file)});
 document.querySelector('#clearBtn')?.addEventListener('click',()=>{if(confirm('Opravdu vymazat všechna data?')){state.machines=[];state.service=[];state.inspections=[];state.logbook=[];state.jobs=[];state.fuel=[];state.shifts=[];save();go('home')}});
}
function render(){const view={home,machines,machineDetail,machineForm,service,inspections,inspectionForm,logbook,logForm,logReport,jobs,jobForm,fuel,fuelForm,shiftStart,shiftActive,shiftEnd,shifts,backup}[state.route]||home;document.getElementById('app').innerHTML=shell(view());bind()}
render();