const KEYS={m:'tm3-machines',s:'tm3-service',i:'tm3-inspections',l:'tm3-logbook',j:'tm6-jobs',f:'tm6-fuel',q:'tm10-shifts',c:'tm11-consumables',sec:'tm13-security',op:'tm22-operators',wt:'tm22-worktypes',lp:'tm22-lastprefs'};
const defaults=[
...
function home(){
 ...
 <div class=home-links><button data-go=jobs>📋 Zakázky</button><button data-go=logbook>📘 Provozní deník</button><button data-go=lists>👷 Číselníky</button></div>
 <div class=footer>TM-Strojník v2.2</div>`;
}
...
function machineTimeline(machineId){
 ...
 state.shifts.filter(x=>x.machineId===machineId).forEach(s=>{
   events.push({
     ...
     text:[s.job,s.work,s.workedHours?`${s.workedHours} MTH`:null].filter(Boolean).join(' · '),
...
function machineDetail(){
 ...
 <div class=detail-links>
   ...
   <button data-machine-settings="${m.id}">⚙️ Nastavení</button>
 </div>
...
function shiftStart(){
 ...
 ${field('job','Zakázka / zákazník','např. Novák – výkop','text',prefs.job||'')}
 ...
 <div class=notice auto-fill-note><b>Automatické doplnění:</b> počáteční stav je převzatý z posledního známého stavu techniky. Údaj můžeš před zahájením ručně opravit.</div>
...
function bind(){
 ...
 const disablePin=document.querySelector('#disablePin');if(disablePin)disablePin.onclick=async()=>{const p=prompt('Zadej současný PIN:');if(p===null)return;if(await hashPin(p)!==state.security.pinHash)return alert('Nesprávný PIN.');state.security={enabled:false,pinHash:'',locked:false};state.operators=[];state.workTypes=[];state.lastPrefs={};save();render()};
 ...
 document.querySelector('#shiftStartForm')?.addEventListener('submit',e=>{e.preventDefault();if(activeShift())return alert('Směna už probíhá.');const f=new FormData(e.target);const machineId=f.get('machineId');const machine=state.machines.find(m=>m.id===machineId);let operator=f.get('operator');if(operator==='__new__')operator=f.get('newOperator').trim();let work=f.get('work');if(work==='__new__')work=f.get('newWork').trim();if(!operator)return alert('Vyber nebo zadej obsluhu.');if(!work)return alert('Vyber nebo zadej druh práce.');rememberListItem('operators',operator);rememberListItem('workTypes',work);const startHours=f.get('startHours')||machine?.hours||'';state.lastPrefs[machineId]={operator,work,job:f.get('job').trim(),place:f.get('place').trim()};state.shifts.unshift({id:crypto.randomUUID(),status:'active',machineId,operator,job:f.get('job').trim(),place:f.get('place').trim(),work,startHours,date:new Date().toISOString().slice(0,10),startedAt:new Date().toISOString(),checks:{fluids:true,hydraulics:true,brakes:true,lights:true,guards:true,site:true}});save();go('shiftActive')});
