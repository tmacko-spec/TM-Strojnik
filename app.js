const KEYS={m:'tm3-machines',s:'tm3-service',i:'tm3-inspections',l:'tm3-logbook'};
const defaults=[
...
const state={route:'home',selected:'',machines:load(KEYS.m,defaults),service:load(KEYS.s,[]),inspections:load(KEYS.i,[]),logbook:load(KEYS.l,[])};

function load(k,f){try{return JSON.parse(localStorage.getItem(k))||structuredClone(f)}catch{return structuredClone(f)}}
function save(){localStorage.setItem(KEYS.m,JSON.stringify(state.machines));localStorage.setItem(KEYS.s,JSON.stringify(state.service));localStorage.setItem(KEYS.i,JSON.stringify(state.inspections));localStorage.setItem(KEYS.l,JSON.stringify(state.logbook))}
function esc(v=''){return String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function go(route,id=''){state.route=route;if(id)state.selected=id;render();scrollTo(0,0)}
function title(){return({home:'TM-Strojník',machines:'Moje stroje',machineForm:'Stroj',service:'Servisní kniha',inspections:'Prohlídky',inspectionForm:'Nová prohlídka',backup:'Záloha dat',logbook:'Provozní deník',logForm:'Nový záznam',logReport:'Výstup pro kontrolu'})[state.route]||'TM-Strojník'}
...
function home(){
 ...
 </div><div class=footer>TM-Strojník v0.5</div>`;
}
...
function bind(){
 ...
 document.querySelector('#exportBtn')?.addEventListener('click',()=>{const blob=new Blob([JSON.stringify({version:'3.1',machines:state.machines,service:state.service,inspections:state.inspections,logbook:state.logbook},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='tm-strojnik-zaloha.json';a.click();URL.revokeObjectURL(a.href)});
 document.querySelector('#importFile')?.addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);state.machines=d.machines||[];state.service=d.service||[];state.inspections=d.inspections||[];state.logbook=d.logbook||[];save();alert('Záloha byla obnovena.');go('home')}catch{alert('Soubor není platný.')}};r.readAsText(file)});
 document.querySelector('#clearBtn')?.addEventListener('click',()=>{if(confirm('Opravdu vymazat všechna data?')){state.machines=[];state.service=[];state.inspections=[];state.logbook=[];save();go('home')}});
}
function render(){const view={home,machines,machineForm,service,inspections,inspectionForm,logbook,logForm,logReport,backup}[state.route]||home;document.getElementById('app').innerHTML=shell(view());bind()}
render();