...
function home(){
 ...
 <div class=footer>TM-Strojník v2.6.4</div>`;
}
...
function machineForm(){
 const m=state.machines.find(x=>x.id===state.selected)||{};
 const editing=Boolean(m.id);
 const category=editing?(m.category||(isVehicle(m)?'vehicle':'machine')):state.newMachineCategory;

 if(!editing&&!category){
   return `<div class=category-step>
    <div class=notice><h3>Co chceš přidat?</h3><p>Nejdříve vyber druh techniky.</p></div>
    <button type=button class=category-big data-new-category=machine><span>🚜</span><b>Stavební stroj</b><small>Bagr, válec, nakladač…</small></button>
    <button type=button class=category-big data-new-category=vehicle><span>🚐</span><b>Automobil</b><small>Dodávka, osobní nebo nákladní auto</small></button>
   </div>`;
 }

 const vehicle=category==='vehicle';
 return `<form id=machineForm><input type=hidden name=id value="${esc(m.id||'')}"><input type=hidden name=category value="${esc(category)}">
 ${editing?`<div class=group><label>Zařazení techniky *</label><select name=categoryEdit id=categoryEdit required><option value=machine ${category==='machine'?'selected':''}>🚜 Stavební stroj</option><option value=vehicle ${category==='vehicle'?'selected':''}>🚐 Automobil</option></select></div>`:`<div class="notice selected-category"><b>${vehicle?'🚐 Automobil':'🚜 Stavební stroj'}</b><button type=button class=link-button data-change-category>Změnit</button></div>`}
 ...
 <div class=grid>
  ${field('serviceInterval',vehicle?'Servisní interval km':'Servisní interval MTH',vehicle?'20000':'500','number',m.serviceInterval||'')}
  ${field('lastServiceHours',vehicle?'Poslední servis při km':'Poslední servis při MTH','0','number',m.lastServiceHours||'')}
 </div>
...
function bind(){
 ...
 function updateMachineCategory(value){const form=document.querySelector('#machineForm');if(!form)return;const vehicle=value==='vehicle';const radio=form.querySelector(`input[name="category"][value="${value}"]`);if(radio)radio.checked=true;form.querySelectorAll('.category-option').forEach(o=>o.classList.toggle('selected',o.dataset.category===value));const typeLabel=form.elements.type?.closest('.group')?.querySelector('label');if(typeLabel)typeLabel.textContent=vehicle?'Typ automobilu':'Typ stavebního stroje';const hoursLabel=form.elements.hours?.closest('.group')?.querySelector('label');if(hoursLabel)hoursLabel.textContent=vehicle?'Kilometry':'Motohodiny';const serialLabel=form.elements.serial?.closest('.group')?.querySelector('label');if(serialLabel)serialLabel.textContent=vehicle?'VIN / SPZ':'Výrobní číslo / VIN';const intervalLabel=form.elements.serviceInterval?.closest('.group')?.querySelector('label');if(intervalLabel)intervalLabel.textContent=vehicle?'Servisní interval km':'Servisní interval MTH';const lastLabel=form.elements.lastServiceHours?.closest('.group')?.querySelector('label');if(lastLabel)lastLabel.textContent=vehicle?'Poslední servis při km':'Poslední servis při MTH';}
 document.querySelectorAll('.category-option').forEach(card=>{card.onclick=e=>{e.preventDefault();updateMachineCategory(card.dataset.category);};});
 document.querySelectorAll('input[name="category"]').forEach(r=>r.onchange=()=>updateMachineCategory(r.value));
 document.querySelectorAll('[data-new-machine]').forEach(x=>x.onclick=()=>{state.selected='';state.newMachineCategory='';go('machineForm')});
 document.querySelectorAll('[data-new-category]').forEach(x=>x.onclick=()=>{state.newMachineCategory=x.dataset.newCategory;render()});
 document.querySelectorAll('[data-change-category]').forEach(x=>x.onclick=()=>{state.newMachineCategory='';render()});
 const categoryEdit=document.querySelector('#categoryEdit');if(categoryEdit){categoryEdit.onchange=()=>{const id=document.querySelector('#machineForm input[name="id"]').value;const m=state.machines.find(x=>x.id===id);if(m){m.category=categoryEdit.value;save();render();}};}
