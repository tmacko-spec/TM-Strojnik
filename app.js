...
function machineStatus(machine){
  ...
  const future=state.inspections
    .filter(x=>x.machineId===machine.id&&x.date)
  ...
  return {level,label,nextInspection:future};
}
...
function home(){
 ...
 return `<div class=stats><div class=stat><strong>${state.machines.length}</strong><span>Stroje</span></div><div class=stat><strong>${outOfService}</strong><span>Mimo provoz / po termínu</span></div><div class=stat><strong>${warning}</strong><span>Vyžadují pozornost</span></div></div><div class=summary-grid><div class=summary-box><small>Servisní záznamy</small><strong>${state.service.length}</strong></div><div class=summary-box><small>Prohlídky do 30 dnů</small><strong>${due}</strong></div><div class=summary-box><small>PHM tento měsíc</small><strong>${fuelThisMonth.toLocaleString('cs-CZ')} Kč</strong></div><div class=summary-box><small>Zakázky</small><strong>${state.jobs.length}</strong></div></div>
 <div class=tiles>
 ...
 </div><div class=footer>TM-Strojník v0.8</div>`;
}
...
function machineCard(m){
 ...
 <div class=machine-overview>
  ...
  <div><small>Náklady PHM</small><b>${fuelCost.toLocaleString('cs-CZ')} Kč</b></div>
 ...
 <div class=actions><button class=dark data-machine-detail="${m.id}">Otevřít kartu</button><button class=fuel-button data-fuel-form="${m.id}">⛽ Tankování</button><button class=secondary data-edit="${m.id}">Upravit</button><button class=danger data-delete="${m.id}">Odstranit</button></div></article>`;
}
function machines(){
