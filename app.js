...
function home(){
 ...
 <div class=footer>TM-Strojník v2.5.1</div>`;
}
...
function bind(){
 ...
 document.querySelectorAll('input[name="category"]').forEach(r=>r.onchange=()=>{const id=document.querySelector('#machineForm input[name="id"]').value;state.selected=id;const form=r.form;const data=new FormData(form);const temp={id,category:r.value,brand:data.get('brand')||'',model:data.get('model')||'',type:data.get('type')||'',year:data.get('year')||'',hours:data.get('hours')||'',serial:data.get('serial')||'',serviceInterval:data.get('serviceInterval')||'',lastServiceHours:data.get('lastServiceHours')||'',note:data.get('note')||''};const index=state.machines.findIndex(m=>m.id===id);if(index>=0)state.machines[index]={...state.machines[index],...temp};render();});
